...
msg = { 'fr':u'Bot: Correction des refs. mal formatées, suppression doublons en utilisant des références nommées (cf. [[Utilisateur:DumZiBoT/liensRefs|explications]])',
        ...
        'en':u'[[tools:~dispenser/view/Reflinks|Reflinks]]: Converting bare references',
    ...
    def process(self, text):
        ...
        for (k, v) in namedRepl.iteritems():
            ...
            if v[1]:
                name = u'"%s"' % name
            text = re.sub(
                ur'<ref name\s*=\s*(?P<quote>"?)\s*%s\s*(?P=quote)\s*/>' % k,
...
class ReferencesRobot:
    ...
    def __init__(self, generator, acceptall=False, limit=None, ignorepdf=False):
        ...
        self.site = pywikibot.getSite()
        self.msg = pywikibot.translate(self.site, msg)
        ...
        for page in self.generator:
            ...
            for match in linksInRef.finditer(
                ...
                try:
                    ...
                    try:
                        req = urllib2.Request(ref.url.decode("utf8"))
                    except UnicodeError:
                        ref.url = urllib2.quote(ref.url.encode("utf8"),"://")
                        req = urllib2.Request(ref.url)
                    req.add_header('User-agent', 'Reflinks/1.4 (Dispenserbot; +http://dispenser.info.tm/~dispenser/view/Reflinks)')
                ...
                if self.titleBlackList.match(ref.title):
                    ...
                    new_text = new_text.replace(match.group(), repl)
                    pywikibot.output(
                        u'\03{lightred}WARNING\03{default} %s : Unusable web page title (%s)'
                        % (ref.link, ref.title))
                    continue
